...
export default function SessionLeftPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-100 dark:bg-slate-900 p-6">
      <div className="max-w-lg w-full p-8 rounded-2xl bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-white/10 text-center">
        ...
        <h1 className="mt-6 text-2xl font-display text-slate-900 dark:text-white">You left the session</h1>
        ...
        {showButtons && (
          <div className="mt-6 flex gap-3">
            ...
            <Link
              href="/"
              className="flex-1 rounded-full border border-slate-300 dark:border-white/20 py-3 text-sm font-semibold text-slate-900 dark:text-white text-center"
            >
              Back to home
            </Link>
          </div>
        ...
        {!showButtons && (
          <div className="mt-6">
            <p className="text-sm text-slate-600 dark:text-slate-400">Thanks for joining — explore other sessions on the homepage.</p>
          </div>
        )}
      </div>
    </div>
